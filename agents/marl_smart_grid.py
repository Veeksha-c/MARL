...
class MARLSmartGrid:  # Main class containing all agents and networks
    ...
    def evaluate_joint_actions(self, states, actions):  # Method to evaluate joint actions using critic
        ...
        states_tensor = torch.FloatTensor(states).unsqueeze(0).to(self.device)  # Convert states
        actions_tensor = torch.FloatTensor(actions).unsqueeze(0).to(self.device)  # Convert actions
